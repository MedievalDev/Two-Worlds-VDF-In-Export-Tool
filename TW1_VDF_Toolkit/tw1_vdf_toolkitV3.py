...
def load_config():
    ...
    defaults = {
        ...
        "default_textures_dir": r"C:/Users/marco/Desktop/TwStuff/WDFiles",
        "last_import_dir":      r"C:/Users/marco/Desktop/TwStuff/WDFiles",
        "last_export_dir": "",
        "csv_last_csv_dir": r"C:/Users/marco/Desktop/TwObjectPosPerTile",
        "csv_last_vdf_dir": r"C:/Users/marco/Desktop/TwStuff/WDFiles",
        "csv_last_out_dir": r"C:/Users/marco/Desktop/TwObjectPosPerTile/Mesh_Export",
        "csv_xls_path":     r"C:/Users/marco/Desktop/TwoWorlds.xls",
    }
...
if not HAS_TK:
    ...
    class VDFToolkitApp:
        ...
        def _create_tabs(self):
            ...
            self.notebook.add(self.tab_csv,    text="  \U0001f4c2  CSV \u2192 Mesh  ")

            self._build_import_tab()
            self._build_export_tab()
            self._build_editor_tab()
            self._build_csv_export_tab()
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # TAB 4 — CSV → MESH EXPORT
    # ══════════════════════════════════════════════════════════════════════════

        _NO_VDF     = '__NO_VDF__'
        _NO_VDF_EXT = {'.prt', '.chv', '.chm', '.hor', '.xfn'}

        def _build_csv_export_tab(self):
            tab = self.tab_csv
            top = ttk.Frame(tab); top.pack(fill="x", padx=10, pady=(10,4))
            top.columnconfigure(1, weight=1)
            def _row(r, label, var, cmd):
                ttk.Label(top, text=label).grid(row=r, column=0, sticky='w', pady=2)
                ttk.Entry(top, textvariable=var, font=("Consolas", 10)).grid(
                    row=r, column=1, sticky='ew', padx=(8,4), pady=2)
                ttk.Button(top, text="Browse", command=cmd).grid(row=r, column=2, pady=2)
            self.csv_dir    = tk.StringVar(value=self.config.get('csv_last_csv_dir',''))
            self.csv_vdfdir = tk.StringVar(value=self.config.get('csv_last_vdf_dir',''))
            self.csv_texdir = tk.StringVar(value=self.config.get('default_textures_dir',''))
            self.csv_outdir = tk.StringVar(value=self.config.get('csv_last_out_dir',''))
            self.csv_xls    = tk.StringVar(value=self.config.get('csv_xls_path',''))
            _row(0,"CSV-Ordner:",      self.csv_dir,    self._csv_browse_csv_dir)
            _row(1,"VDF-Quellordner:", self.csv_vdfdir, self._csv_browse_vdf_dir)
            _row(2,"Textur-Ordner:",   self.csv_texdir, self._csv_browse_tex_dir)
            _row(3,"Output-Ordner:",   self.csv_outdir, self._csv_browse_output)
            _row(4,"XLS-Mapping:",     self.csv_xls,    self._csv_browse_xls)

            scan_row = ttk.Frame(tab); scan_row.pack(fill="x", padx=10, pady=(2,2))
            ttk.Button(scan_row, text="\U0001f50d  CSVs scannen",
                       command=self._csv_scan).pack(side="left")
            self.csv_hide_novdf = tk.BooleanVar(value=True)
            ttk.Checkbutton(scan_row, text="Partikel/Sound ausblenden",
                            variable=self.csv_hide_novdf,
                            command=self._csv_apply_filter).pack(side="left", padx=8)
            self.csv_show_missing_only = tk.BooleanVar(value=False)
            ttk.Checkbutton(scan_row, text="Nur fehlende",
                            variable=self.csv_show_missing_only,
                            command=self._csv_apply_filter).pack(side="left", padx=4)
            self.csv_scan_label = ttk.Label(scan_row, text="")
            self.csv_scan_label.pack(side="left", padx=6)

            mid = ttk.Frame(tab); mid.pack(fill="both", expand=True, padx=10, pady=4)
            cols = ('mesh','category','resolved','vdf_found','status')
            self.csv_tree = ttk.Treeview(mid, columns=cols, show='headings',
                                          selectmode='extended')
            self.csv_tree.heading('mesh',      text='CSV Mesh Name')
            self.csv_tree.heading('category',  text='Kategorie')
            self.csv_tree.heading('resolved',  text='Aufgelöst als')
            self.csv_tree.heading('vdf_found', text='VDFs')
            self.csv_tree.heading('status',    text='Status')
            self.csv_tree.column('mesh',      width=220, minwidth=140)
            self.csv_tree.column('category',  width=90,  anchor='center')
            self.csv_tree.column('resolved',  width=240, minwidth=120)
            self.csv_tree.column('vdf_found', width=65,  anchor='center')
            self.csv_tree.column('status',    width=280, minwidth=120)
            vsb = ttk.Scrollbar(mid, orient='vertical', command=self.csv_tree.yview)
            self.csv_tree.configure(yscrollcommand=vsb.set)
            self.csv_tree.pack(side='left', fill='both', expand=True)
            vsb.pack(side='right', fill='y')

            btn = ttk.Frame(tab); btn.pack(fill="x", padx=10, pady=4)
            self.csv_progress = ttk.Progressbar(btn, mode='determinate',
                                                  style="TProgressbar")
            self.csv_progress.pack(fill="x", pady=(0,4))
            self.csv_progress_label = ttk.Label(btn, text="")
            self.csv_progress_label.pack(side="left")
            self.csv_btn_cancel = ttk.Button(btn, text="Cancel",
                                              command=self._csv_cancel, state='disabled')
            self.csv_btn_cancel.pack(side="right", padx=4)
            self.csv_btn_start = ttk.Button(btn, text="\u25B6  Export starten",
                                             style="Accent.TButton",
                                             command=self._csv_start)
            self.csv_btn_start.pack(side="right", padx=4)
            ttk.Button(btn, text="\U0001f4c4  CSVs schreiben",
                       command=self._csv_write_cleaned).pack(side="right", padx=4)

            log_f = ttk.Frame(tab); log_f.pack(fill="x", padx=10, pady=(0,6))
            self.csv_log_text = tk.Text(log_f, height=5, bg=BG2, fg=FG,
                                         font=("Consolas",9), relief="flat",
                                         wrap="word", insertbackground=FG)
            self.csv_log_text.pack(fill="x")
            self.csv_log_text.configure(state='disabled')
            self.csv_jobs = []
            self._csv_raw_files  = {}
            self._csv_job_index  = {}

        # ── XLS-Mapping ──────────────────────────────────────────────────────
        def _csv_build_xls_mapping(self, xls_path):
            try:
                import xlrd
            except ImportError:
                self._csv_log("  HINWEIS: xlrd fehlt — pip install xlrd", YELLOW); return {}
            if not xls_path or not os.path.isfile(xls_path): return {}
            try:
                wb = xlrd.open_workbook(xls_path)
            except Exception as e:
                self._csv_log(f"  XLS-Fehler: {e}", RED); return {}
            import re
            def expand(s):
                m = re.search(r'\[(\d+)-(\d+)\]', s)
                if not m: return [s]
                lo,hi = int(m.group(1)),int(m.group(2))
                return [s[:m.start()]+str(i)+s[m.end():] for i in range(lo,hi+1)]
            mapping = {}
            for sheet_name in wb.sheet_names():
                ...
                for r in range(min(5, sh.nrows)):
                    ...
                    if any('objid' in v for v in row) and any(v=='mesh' for v in row):
                        header_row = r; break
                if header_row is None: continue
                headers = [str(sh.cell_value(header_row,c)).strip().lower()
                           for c in range(sh.ncols)]
                try:
                    id_col   = next(i for i,h in enumerate(headers) if 'objid' in h)
                    mesh_col = headers.index('mesh')
                except (ValueError, StopIteration): continue
                for r in range(header_row+1, sh.nrows):
                    obj_id = str(sh.cell_value(r, id_col)).strip()
                    mesh   = str(sh.cell_value(r, mesh_col)).strip()
                    if not obj_id or not mesh: continue
                    if obj_id[0] in ('#',';','?') or obj_id[:3]=='ID_': continue
                    if mesh.lower() in ('0','null','#empty_mesh',''): continue
                    mesh_clean = mesh.split(':')[0].strip()
                    raw = mesh_clean.replace('\\\\','\\')
                    basename = re.split(r'[/\\]', raw)[-1].split('@')[0]
                    ext  = os.path.splitext(basename)[1].lower()
                    stem = os.path.splitext(basename)[0]
                    if not stem or stem.lower() in ('0','null',''): continue
                    if ext in self._NO_VDF_EXT or mesh.strip().startswith('#EMPTY'):
                        if obj_id.upper() not in mapping:
                            mapping[obj_id.upper()] = [self._NO_VDF]
                        continue
                    if obj_id.upper() not in mapping:
                        mapping[obj_id.upper()] = expand(stem)
            return mapping

        # ── Einzel-Token Lookup → Liste von (name, path) ─────────────────────
        def _csv_lookup_token(self, token, vdf_index, xls_mapping):
            import re
            clean = re.sub(r'\(.*$','',token).strip()
            if not clean or clean.upper()=='NULL': return []
            slot_m   = re.match(r'[A-Za-z][A-Za-z0-9_]+\((\d+)', token)
            slot_num = int(slot_m.group(1)) if slot_m else None
            def _try_one(stem):
                base = stem.rstrip('_'); k = base.upper()
                if k in vdf_index: return (base, vdf_index[k])
                if slot_num is not None:
                    for cand in (f"{base}_{slot_num:02d}",
                                 f"{base}_{slot_num:03d}",
                                 f"{base}_{slot_num}"):
                        if cand.upper() in vdf_index:
                            return (cand, vdf_index[cand.upper()])
                return None
            xls_stems = xls_mapping.get(clean.upper(), [])
            if xls_stems == [self._NO_VDF]:
                return [(self._NO_VDF, None)]
            if xls_stems:
                results=[]; seen_p=set()
                for stem in xls_stems:
                    hit = _try_one(stem)
                    if hit and str(hit[1]) not in seen_p:
                        seen_p.add(str(hit[1])); results.append(hit)
                if results: return results
            hit = _try_one(clean)
            if hit: return [hit]
            stripped = re.sub(r'(\d+)[a-zA-Z]+',r'\1',clean)
            if stripped.upper() != clean.upper():
                hit = _try_one(stripped)
                if hit: return [hit]
            return []

        # ── Haupt-Resolver ────────────────────────────────────────────────────
        def _csv_resolve_vdf(self, csv_name, vdf_index, xls_mapping):
            import re
            tokens = re.findall(r'[A-Za-z][A-Za-z0-9_]*(?:\([^)]*\))?', csv_name)
            seen=set(); results=[]; seen_paths=set()
            for token in tokens:
                clean = re.sub(r'\(.*$','',token).strip()
                if not clean or clean.upper()=='NULL' or clean.upper() in seen: continue
                seen.add(clean.upper())
                for name,path in self._csv_lookup_token(token, vdf_index, xls_mapping):
                    if name==self._NO_VDF:
                        if self._NO_VDF not in seen_paths:
                            seen_paths.add(self._NO_VDF); results.append((self._NO_VDF,None))
                    elif str(path) not in seen_paths:
                        seen_paths.add(str(path)); results.append((name,path))
            return results

        # ── Filter anwenden ───────────────────────────────────────────────────
        def _csv_apply_filter(self):
            hide_novdf  = self.csv_hide_novdf.get()
            only_miss   = self.csv_show_missing_only.get()
            for csv_name,category,vdf_hits,iid,is_novdf in self.csv_jobs:
                is_missing  = (not vdf_hits and not is_novdf)
                # Sichtbarkeit bestimmen
                show = True
                if hide_novdf and is_novdf:   show = False
                if only_miss  and not is_missing: show = False
                currently = self.csv_tree.exists(iid)
                if show and not currently:    self.csv_tree.reattach(iid,'','end')
                elif not show and currently:  self.csv_tree.detach(iid)

        # ── CSVs schreiben ────────────────────────────────────────────────────
        def _csv_write_cleaned(self):
            if not self._csv_raw_files:
                messagebox.showwarning("Kein Scan","Bitte zuerst 'CSVs scannen' ausführen."); return
            output = self.csv_outdir.get()
            if not output:
                messagebox.showwarning("Kein Output","Bitte Output-Ordner wählen."); return
            import csv as _csv
            dir_clean   = os.path.join(output, "Cleaned_CSVs")
            dir_missing = os.path.join(output, "Missing_CSVs")
            os.makedirs(dir_clean,   exist_ok=True)
            os.makedirs(dir_missing, exist_ok=True)
            cnt_clean=0; cnt_miss=0; cnt_files=0
            for csv_filename, file_data in self._csv_raw_files.items():
                fieldnames = file_data['fieldnames']; rows = file_data['rows']
                clean_rows=[]; missing_rows=[]
                for orig_name, row_dict in rows:
                    job_info = self._csv_job_index.get(orig_name.upper())
                    if job_info is None: missing_rows.append(dict(row_dict)); continue
                    vdf_hits, is_novdf = job_info
                    if is_novdf: continue
                    if not vdf_hits:
                        missing_rows.append(dict(row_dict))
                    else:
                        new_row = dict(row_dict)
                        new_row['MeshName'] = vdf_hits[0][0]
                        clean_rows.append(new_row)
                if clean_rows:
                    with open(os.path.join(dir_clean,csv_filename),'w',
                              newline='',encoding='utf-8-sig') as f:
                        w=_csv.DictWriter(f,fieldnames=fieldnames); w.writeheader(); w.writerows(clean_rows)
                    cnt_clean+=len(clean_rows); cnt_files+=1
                if missing_rows:
                    with open(os.path.join(dir_missing,csv_filename),'w',
                              newline='',encoding='utf-8-sig') as f:
                        w=_csv.DictWriter(f,fieldnames=fieldnames); w.writeheader(); w.writerows(missing_rows)
                    cnt_miss+=len(missing_rows)
            self._csv_log(
                f"\nCSVs geschrieben:\n  Cleaned_CSVs/ → {cnt_clean} Zeilen\n"
                f"  Missing_CSVs/ → {cnt_miss} Zeilen", GREEN)
            self._status(f"CSVs: {cnt_clean} bereinigt  |  {cnt_miss} fehlend", GREEN)
            messagebox.showinfo("CSVs geschrieben",
                f"Cleaned_CSVs/  →  {cnt_clean} Zeilen\n"
                f"Missing_CSVs/  →  {cnt_miss} Zeilen\n\nAusgabe: {output}")

        def _csv_log(self, msg, color=None):
            self.csv_log_text.configure(state='normal')
            start = self.csv_log_text.index('end-1c')
            self.csv_log_text.insert('end', msg+'\n')
            if color:
                end = self.csv_log_text.index('end-1c')
                tag = f"col_{color.replace('#','')}"
                self.csv_log_text.tag_configure(tag, foreground=color)
                self.csv_log_text.tag_add(tag, start, end)
            self.csv_log_text.see('end')
            self.csv_log_text.configure(state='disabled')

        def _csv_browse_csv_dir(self):
            p = filedialog.askdirectory(title="Ordner mit CSV-Dateien wählen")
            if p:
                self.csv_dir.set(p)
                if not self.csv_outdir.get(): self.csv_outdir.set(os.path.join(p,"Mesh_Export"))
        def _csv_browse_vdf_dir(self):
            p = filedialog.askdirectory(title="VDF-Quellordner (rekursiv)")
            if p:
                self.csv_vdfdir.set(p)
                if not self.csv_texdir.get():
                    tex = find_textures_folder(p); 
                    if tex: self.csv_texdir.set(tex)
        def _csv_browse_tex_dir(self):
            p = filedialog.askdirectory(title="Textur-Ordner"); 
            if p: self.csv_texdir.set(p)
        def _csv_browse_output(self):
            p = filedialog.askdirectory(title="Output-Ordner"); 
            if p: self.csv_outdir.set(p)
        def _csv_browse_xls(self):
            p = filedialog.askopenfilename(title="TwoWorlds.xls",
                filetypes=[("Excel","*.xls *.xlsx"),("Alle","*.*")])
            if p: self.csv_xls.set(p)

        def _csv_scan(self):
            csv_dir = self.csv_dir.get(); vdf_dir = self.csv_vdfdir.get()
            if not csv_dir or not os.path.isdir(csv_dir):
                messagebox.showwarning("Fehler","Bitte einen gültigen CSV-Ordner wählen."); return
            cat_map = {'foliage':'Foliage','static':'Static','interactive':'Interactive'}
            import csv as _csv
            self._csv_raw_files = {}; mesh_set = {}
            csv_files = list(Path(csv_dir).glob("*.csv"))
            if not csv_files:
                messagebox.showwarning("Keine CSVs","Keine .csv-Dateien gefunden."); return
            for csv_path in csv_files:
                sl = csv_path.stem.lower(); category = None
                for key,cat in cat_map.items():
                    if sl.endswith(f'_{key}'): category=cat; break
                if not category:
                    self._csv_log(f"  Übersprungen: {csv_path.name}", YELLOW); continue
                try:
                    with open(csv_path, newline='', encoding='utf-8-sig') as f:
                        reader = _csv.DictReader(f)
                        rows_raw = list(reader); fieldnames = reader.fieldnames or []
                    file_rows = []
                    for row in rows_raw:
                        name = row.get('MeshName','').strip()
                        if not name: continue
                        mesh_set[(name, category)] = True
                        file_rows.append((name, row))
                    self._csv_raw_files[csv_path.name] = {'fieldnames':fieldnames,'rows':file_rows}
                except Exception as e:
                    self._csv_log(f"  CSV-Fehler {csv_path.name}: {e}", RED)

            vdf_index = {}; vdf_raw_count = 0
            if vdf_dir and os.path.isdir(vdf_dir):
                for dp,_,fnames in os.walk(vdf_dir):
                    for fn in fnames:
                        if fn.upper().endswith('.VDF'):
                            vdf_raw_count += 1
                            k = Path(fn).stem.upper()
                            if k not in vdf_index: vdf_index[k] = Path(dp)/fn
            if vdf_raw_count==0 and vdf_dir:
                self._csv_log(f"  WARNUNG: Keine .vdf in '{vdf_dir}'!", RED)
            else:
                self._csv_log(f"VDF-Index: {vdf_raw_count} Dateien in '{vdf_dir}'")

            xls_path = self.csv_xls.get()
            self._csv_log(f"Lade XLS: {os.path.basename(xls_path)} ...")
            xls_mapping = self._csv_build_xls_mapping(xls_path)
            if xls_mapping:
                novdf_in_xls = sum(1 for v in xls_mapping.values() if v==[self._NO_VDF])
                self._csv_log(f"  {len(xls_mapping)} ObjID-Mappings  ({novdf_in_xls} Partikel/Sound)", GREEN)
            else:
                self._csv_log("  Kein XLS-Mapping aktiv", YELLOW)

            self.csv_tree.delete(*self.csv_tree.get_children())
            self.csv_jobs=[]; self._csv_job_index={}
            found_count=miss_count=novdf_count=xls_count=0
            hide_novdf = self.csv_hide_novdf.get()
            only_miss  = self.csv_show_missing_only.get()
            ORANGE = "#FFA040"

            for (csv_name,category) in sorted(mesh_set.keys()):
                hits     = self._csv_resolve_vdf(csv_name, vdf_index, xls_mapping)
                vdf_hits = [(n,p) for n,p in hits if n!=self._NO_VDF]
                novdf_hit= any(n==self._NO_VDF for n,_ in hits)
                is_novdf = novdf_hit and not vdf_hits
                is_missing = not hits or (not vdf_hits and not is_novdf)
                self._csv_job_index[csv_name.upper()] = (vdf_hits, is_novdf)

                if not hits:
                    resolved_str="—"; found_str="✘"; miss_count+=1
                elif is_novdf:
                    resolved_str="Partikel / Sound"; found_str="—"; novdf_count+=1
                else:
                    resolved_str="  |  ".join(n for n,_ in vdf_hits)
                    found_str = f"✔ {len(vdf_hits)}" if len(vdf_hits)>1 else "✔"
                    found_count+=1
                    import re
                    tokens = re.findall(r'[A-Za-z][A-Za-z0-9_]*(?:\([^)]*\))?',csv_name)
                    unique = set(re.sub(r'\(.*$','',t).strip().upper() for t in tokens
                                 if re.sub(r'\(.*$','',t).strip().upper() not in ('','NULL'))
                    if any(xls_mapping.get(c,[])!=[self._NO_VDF] and xls_mapping.get(c)
                           for c in unique): xls_count+=1

                iid = self.csv_tree.insert('','end',
                    values=(csv_name,category,resolved_str,found_str,"Bereit"))
                if not hits:
                    self.csv_tree.tag_configure('miss',foreground=RED)
                    self.csv_tree.item(iid,tags=('miss',))
                elif is_novdf:
                    self.csv_tree.tag_configure('novdf',foreground=ORANGE)
                    self.csv_tree.item(iid,tags=('novdf',))

                # Filter sofort anwenden
                show = True
                if hide_novdf and is_novdf:    show = False
                if only_miss  and not is_missing: show = False
                if not show: self.csv_tree.detach(iid)

                self.csv_jobs.append((csv_name,category,vdf_hits,iid,is_novdf))

            total = len(self.csv_jobs)
            hidden_novdf = f"  |  {novdf_count} ausgeblendet" if hide_novdf and novdf_count else ""
            self.csv_scan_label.configure(
                text=(f"{total} Meshes  |  {found_count} OK  |  {miss_count} fehlend  "
                      f"|  {xls_count} XLS  |  {novdf_count} Partikel{hidden_novdf}"))
            self._csv_log(
                f"Scan: {len(csv_files)} CSV(s) → {total} Meshes  "
                f"| {found_count} OK  | {miss_count} fehlend  | {novdf_count} Partikel/Sound", GREEN)

        def _csv_cancel(self): self.cancel_flag = True

        def _csv_start(self):
            if not self.csv_jobs:
                self._csv_scan()
                if not self.csv_jobs:
                    messagebox.showwarning("Keine Jobs","Zuerst CSVs scannen."); return
            output = self.csv_outdir.get()
            if not output:
                messagebox.showwarning("Kein Output","Bitte Output-Ordner wählen."); return
            active_jobs = [(n,c,h,i,nv) for n,c,h,i,nv in self.csv_jobs if h]
            if not active_jobs:
                messagebox.showwarning("Keine VDFs","Keine VDF-Dateien gefunden."); return
            self.config.update({'csv_last_csv_dir':self.csv_dir.get(),
                                 'csv_last_vdf_dir':self.csv_vdfdir.get(),
                                 'csv_last_out_dir':output,
                                 'csv_xls_path':self.csv_xls.get()})
            save_config(self.config)
            self.cancel_flag=False
            self.csv_btn_start.configure(state='disabled')
            self.csv_btn_cancel.configure(state='normal')
            self.csv_progress['value']=0; self.csv_progress['maximum']=len(active_jobs)
            self.csv_log_text.configure(state='normal')
            self.csv_log_text.delete('1.0','end')
            self.csv_log_text.configure(state='disabled')
            tex_index={}
            tex_dir = self.csv_texdir.get()
            if tex_dir and os.path.isdir(tex_dir):
                self._csv_log(f"Scanne Texturen: {tex_dir} ...")
                tex_index = build_texture_index(tex_dir)
                self._csv_log(f"  {len(tex_index)} DDS indiziert", GREEN)
            for csv_name,cat,hits,iid,nv in self.csv_jobs:
                if not hits and not nv:
                    self.csv_tree.set(iid,'status',"nicht gefunden — übersprungen")
            self._csv_log(f"\nExport: {len(active_jobs)} Meshes → {output}")
            counters={'ok':0,'err':0,'tex_found':0,'tex_miss':0}
            self.root.after(1, self._csv_batch, 0, active_jobs, tex_index, output, counters)

        def _csv_batch(self, idx, jobs, tex_index, output, counters):
            if self.cancel_flag or idx>=len(jobs):
                self.csv_btn_start.configure(state='normal')
                self.csv_btn_cancel.configure(state='disabled')
                self.csv_progress_label.configure(text="")
                sep="═"*54; self._csv_log(f"\n{sep}")
                if self.cancel_flag:
                    self._csv_log(f"Abgebrochen {idx}/{len(jobs)}  {counters['ok']} OK  {counters['err']} Fehler",YELLOW)
                    self._status(f"Abgebrochen: {counters['ok']} OK",YELLOW)
                else:
                    self._csv_log(
                        f"Fertig!  {counters['ok']} exportiert  {counters['err']} Fehler  "
                        f"{counters['tex_found']} Tex  {counters['tex_miss']} fehlend",GREEN)
                    self._status(f"CSV→Mesh: {counters['ok']} OK  {counters['err']} Fehler",GREEN)
                self._csv_log(sep); return
            csv_name,category,hits,iid,_ = jobs[idx]
            self.csv_tree.set(iid,'status',"Konvertiere...")
            self.csv_progress['value']=idx+1
            pct=(idx+1)*100//len(jobs)
            self.csv_progress_label.configure(text=f"{csv_name}  [{idx+1}/{len(jobs)}  {pct}%]")
            self.root.update_idletasks()
            sub_dir=os.path.join(output,"Meshes",category); os.makedirs(sub_dir,exist_ok=True)
            job_ok=True; parts=[]
            for resolved_name,vdf_path in hits:
                lod_path=None
                lod_cand=Path(vdf_path).parent/f"{Path(vdf_path).stem}_LOD.vdf"
                if not lod_cand.exists():
                    for f in Path(vdf_path).parent.iterdir():
                        if (f.suffix.upper()=='.VDF' and
                                f.stem.upper()==f"{Path(vdf_path).stem.upper()}_LOD"):
                            lod_cand=f; break
                if lod_cand.exists(): lod_path=lod_cand
                try:
                    self._csv_log(f"\n[{category}] {csv_name}  \u2192  {resolved_name}")
                    _,stats=convert_vdf_to_obj(str(vdf_path),str(lod_path) if lod_path else None,
                                               sub_dir,self._csv_log,tex_index=tex_index,metadata_dir=None)
                    counters['tex_found']+=stats.get('tex_found',0)
                    counters['tex_miss'] +=stats.get('tex_missing',0)
                    parts.append(f"{resolved_name}: {stats['base_verts']}v"+(" +LOD" if stats.get('has_lod') else ""))
                except Exception as e:
                    self._csv_log(f"  FEHLER ({resolved_name}): {e}",RED)
                    parts.append(f"{resolved_name}: FEHLER"); job_ok=False
            if job_ok:
                self.csv_tree.set(iid,'status',"  |  ".join(parts)); counters['ok']+=1
            else:
                self.csv_tree.set(iid,'status',"FEHLER  "+"  |  ".join(parts)); counters['err']+=1
            self.root.after(1,self._csv_batch,idx+1,jobs,tex_index,output,counters)
